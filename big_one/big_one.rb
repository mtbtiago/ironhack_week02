...
post '/login' do
  # puts params
  #  puts "usuario "+params[:user]
  #  puts "contraseña "+params[:passw]
  @user = big_one.validate_and_return_user(params[:user],params[:passw])
  if !@user
    redirect('/')
  else
    big_one.user = @user
...
post '/add_shout' do
  @shout = Shout.new(
  	message: params[:shout], 
  	user: big_one.user, 
  	created_at: Date.new,
  	likes: 0)
  # @shout.valid?
  @shout.save
  # puts @shout.errors.full_messages
end
